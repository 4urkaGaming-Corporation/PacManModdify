import pygame
from settings import WIDTH, HEIGHT, BLUE, WHITE, BLACK
...
def show_info():
    ...
    while running:
        ...
        for i, line in enumerate(info_lines):
            ...
            window.blit(
                text_surface,
                (WIDTH // 2 - text_surface.get_width() // 2, start_y + i * 30)
            )
        back_text = font.render("Натисніть ESC, щоб повернутися назад", True, (0, 255, 0))
        window.blit(
            back_text,
            (WIDTH // 2 - back_text.get_width() // 2, start_y + len(info_lines) * 30 + 20)
        )
        pygame.display.flip()
