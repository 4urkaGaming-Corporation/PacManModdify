...
def test_game_score_update(game, mocker): 
    game.maze.coins = [] 
    game.score = 10 * (game.initial_coin_count - len(game.maze.coins)) 
    assert game.score == 10 * game.initial_coin_count 
 
def test_game_over_collision(game, mocker): 
    mocker.patch.object(game.pacman, 'get_rect', return_value=pygame.Rect(50, 50, 20, 20)) 
    mocker.patch.object(game.enemies[0], 'get_rect', return_value=pygame.Rect(50, 50, 20, 20)) 
    game.run() 
